 ...
 */
class tx_jftabulatorsitemap_tceFunc
{
	/**
	...
	public function getCheckbox($PA, &$fObj)
	{
		...
		if ($confArr['useSelectInsteadCheckbox']) {
			$PA['fieldConf']['config'] = array(
				'type' => 'select',
				'items' => array(
					array('LLL:EXT:jftabulatorsitemap/locallang_db.xml:tt_content.pi_flexform.from_ts', 2),
					array('LLL:EXT:jftabulatorsitemap/locallang_db.xml:tt_content.pi_flexform.yes',1),
					array('LLL:EXT:jftabulatorsitemap/locallang_db.xml:tt_content.pi_flexform.no',0),
				),
			);
		} else {
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/jftabulatorsitemap/lib/class.tx_jftabulatorsitemap_tceFunc.php']) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/jftabulatorsitemap/lib/class.tx_jftabulatorsitemap_tceFunc.php']);
}
?>